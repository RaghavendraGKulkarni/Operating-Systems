...
void push(heap *next, process p) {
    ...
    while(!heap && index > 0) {
        ...
        if(next->heapArray[parent].arrivalTime < next->heapArray[index].arrivalTime || (next->heapArray[parent].arrivalTime == next->heapArray[index].arrivalTime && next->heapArray[parent].id < next->heapArray[index].id))
            heap = true;
        else {
...
process pop(heap *next) {
    ...
    while(!heap && 2*index + 1 < next->heapSize) {
        child = 2*index + 1;
        if(child + 1 < next->heapSize && (next->heapArray[child + 1].arrivalTime < next->heapArray[child].arrivalTime || (next->heapArray[child + 1].arrivalTime == next->heapArray[child].arrivalTime && next->heapArray[child + 1].id < next->heapArray[child].id)))
            child++;
        if(next->heapArray[child].arrivalTime < next->heapArray[index].arrivalTime || (next->heapArray[child].arrivalTime == next->heapArray[index].arrivalTime && next->heapArray[child].id < next->heapArray[index].id))
            heap = true;
        else {
